          ...
          <TabsContent value="timeline" className="space-y-4">
            {loading ? (
              ...
              <div className="space-y-4">
                {[...changeHistory, ...paymentHistory]
                  .sort((a, b) => new Date(b.created_at || b.change_date).getTime() - 
                                  new Date(a.created_at || a.change_date).getTime())
                  .map((item, index) => {
                    const isPayment = 'amount_paid' in item;
                    const date = isPayment ? item.created_at : item.change_date;
                    
                    return (
                      <div key={index} className="flex items-start space-x-3 p-3 bg-white border rounded-lg">
                        {getChangeTypeIcon(isPayment ? 'payment' : item.change_type)}
                        <div className="flex-1">
                          <div className="flex items-center space-x-2 mb-1">
                            <Badge className={getChangeTypeBadge(isPayment ? 'payment' : item.change_type)}>
                              {isPayment ? 'Payment' : item.change_type.charAt(0).toUpperCase() + item.change_type.slice(1)}
                            </Badge>
                          ...
                          {isPayment ? (
                            <div className="space-y-1">
                              <div className="font-medium text-green-600">
                                Payment of {formatCurrency(item.amount_paid)}
                              </div>
                              <div className="text-sm text-gray-600">
                                Method: {item.payment_method} | Receipt: {item.receipt_number}
                              </div>
                              <div className="text-sm text-gray-600">
                                Received by: {item.payment_receiver}
                              </div>
                              {item.notes && (
                            ...
                            <div className="space-y-1">
                              <div className="font-medium">
                                {item.change_type === 'discount' && 
                                  `Discount applied: ${formatCurrency(item.amount || 0)}`
                                }
                                {item.change_type === 'creation' && 
                                  `Fee record created: ${formatCurrency(item.amount || 0)}`
                                }
