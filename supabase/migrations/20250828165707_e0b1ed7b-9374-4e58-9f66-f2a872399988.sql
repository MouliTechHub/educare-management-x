-- Update students table to support proper status lifecycle
-- First, let's modify the status constraint to allow the new values
...
-- Create views for different student statuses
CREATE OR REPLACE VIEW public.v_students_by_status AS
SELECT 
  s.*,
...
-- Create specific views for each status
CREATE OR REPLACE VIEW public.v_active_students_new AS
SELECT * FROM public.v_students_by_status WHERE status = 'Active';

CREATE OR REPLACE VIEW public.v_inactive_students_new AS
SELECT * FROM public.v_students_by_status WHERE status = 'Inactive';

CREATE OR REPLACE VIEW public.v_alumni_students AS
SELECT * FROM public.v_students_by_status WHERE status = 'Alumni';

CREATE OR REPLACE VIEW public.v_transferred_students AS
SELECT * FROM public.v_students_by_status WHERE status = 'Transferred';

CREATE OR REPLACE VIEW public.v_withdrawn_students AS
SELECT * FROM public.v_students_by_status WHERE status = 'Withdrawn';
...
ALTER VIEW public.v_students_by_status OWNER TO postgres;
ALTER VIEW public.v_active_students_new OWNER TO postgres;
ALTER VIEW public.v_inactive_students_new OWNER TO postgres;
ALTER VIEW public.v_alumni_students OWNER TO postgres;
ALTER VIEW public.v_transferred_students OWNER TO postgres;
ALTER VIEW public.v_withdrawn_students OWNER TO postgres;