...
-- Add constraints for aadhaar_number (without IF NOT EXISTS which is not supported for constraints)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'aadhaar_number_check') THEN
        ALTER TABLE public.teachers ADD CONSTRAINT aadhaar_number_check CHECK (aadhaar_number IS NULL OR aadhaar_number ~ '^[0-9]{12}$');
    END IF;
END $$;

DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'aadhaar_number_unique') THEN
        ALTER TABLE public.teachers ADD CONSTRAINT aadhaar_number_unique UNIQUE(aadhaar_number);
    END IF;
END $$;
...
-- Add trigger to update updated_at columns (only if they don't exist)
DO $$ 
BEGIN
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_teacher_salaries_updated_at') THEN
