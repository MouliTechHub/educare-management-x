-- Fix duplicate payment allocation issue by adding ON CONFLICT handling
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.payment_allocations WHERE payment_record_id = p_payment_record_id) THEN
    -- Return existing allocation data
    SELECT jsonb_build_object(
      'total_allocated', COALESCE(SUM(allocated_amount), 0),
      'remaining_amount', p_payment_amount - COALESCE(SUM(allocated_amount), 0),
