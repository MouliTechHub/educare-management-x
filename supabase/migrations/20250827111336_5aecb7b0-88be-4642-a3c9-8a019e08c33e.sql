...
-- 2) Add new lifecycle columns to students table
...
-- 4) Views for different student states (using text status for now)
...
-- 5) RPC functions for status management (using text status)
...
BEGIN
  ...
  IF p_new_status NOT IN ('Active', 'Inactive', 'Alumni', 'Transferred', 'Withdrawn') THEN
    RAISE EXCEPTION 'invalid_status: must be one of Active, Inactive, Alumni, Transferred, Withdrawn';
  END IF;
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION public.set_student_status(uuid, text, text, text, date, bool) TO authenticated;
