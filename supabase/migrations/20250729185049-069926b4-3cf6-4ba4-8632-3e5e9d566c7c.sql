...
CREATE OR REPLACE FUNCTION public.has_any_role(check_roles app_role[], user_uuid UUID DEFAULT auth.uid())
RETURNS BOOLEAN
LANGUAGE SQL
SECURITY DEFINER
STABLE
...
FOR SELECT 
USING (
  public.has_any_role(ARRAY['admin', 'teacher', 'accountant']::app_role[]) OR
...
FOR SELECT 
USING (
  public.has_any_role(ARRAY['admin', 'accountant']::app_role) OR
...
CREATE POLICY "Admins and accountants can modify fee records" 
ON public.student_fee_records 
FOR ALL 
...
-- Secure database functions by adding search_path
