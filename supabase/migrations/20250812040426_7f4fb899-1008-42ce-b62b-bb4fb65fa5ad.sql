-- Fix ambiguous idempotency_key by renaming function parameter and qualifying column
...
BEGIN
  -- Compute idempotency key from param or derived key
  computed_key := COALESCE(p_idempotency_key, md5(promotion_data::TEXT || target_academic_year_id::TEXT || promoted_by_user));

  -- Look up existing request by fully-qualified column name to avoid ambiguity
  ...
  LOOP
    ...
    -- Outstanding balance for previous year
