-- Harden promotion RPC with before/after snapshots and safer data-fix
...
BEGIN
  ...
  -- Recalculate balance_fee for source and target years (source remains immutable except for recompute)
  ...
  WHERE academic_year_id IN (
    SELECT DISTINCT (p.item->>'from_academic_year_id')::UUID
    FROM jsonb_array_elements(promotion_data) AS p(item)
    UNION
    SELECT target_academic_year_id
  );
...
-- Recalculate balance_fee after cleanup
UPDATE public.student_fee_records
SET balance_fee = GREATEST(
  COALESCE(actual_fee, 0) - COALESCE(paid_amount, 0) - COALESCE(discount_amount, 0), 
  0
