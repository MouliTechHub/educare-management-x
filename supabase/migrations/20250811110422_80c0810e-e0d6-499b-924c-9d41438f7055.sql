-- Create a robust, single-source discount application RPC and a uniqueness guard to prevent duplicate history rows

-- 1) Unique index to prevent double inserts within same second for identical event signature
CREATE UNIQUE INDEX IF NOT EXISTS ux_discount_history_src_student_amt_by_time
ON public.discount_history (
  COALESCE(source_fee_id, fee_id),
...
-- 2) Transactional RPC to apply a discount and log once
...
BEGIN
  ...
  -- Insert discount history, tolerate uniqueness race (e.g., existing trigger)
  BEGIN
    IF NOT EXISTS (
  ...
  EXCEPTION WHEN unique_violation THEN
    -- Ignore; another process (e.g., trigger) already inserted the same logical event
    NULL;
  END;
